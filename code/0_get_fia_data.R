...
files.dt <- data.table(url = c('https://apps.fs.usda.gov/fia/datamart/CSV/ID_TREE.csv',
                               ...
                               'https://apps.fs.usda.gov/fia/datamart/CSV/ID_COND.csv',
                               'https://apps.fs.usda.gov/fia/datamart/CSV/OR_COND.csv',
                               'https://apps.fs.usda.gov/fia/datamart/CSV/WA_COND.csv'),
                       file = c('ID_TREE.csv',
                                'OR_TREE.csv',
                                'WA_TREE.csv',
                                'ID_PLOT.csv',
...
cond.dt <- do.call("rbind", lapply(list.files('data/fia/state/', pattern = 'COND.csv', full.names = T), fread))

# write out
fwrite(trees.dt, 'data/fia/WAORID_fia_tree_surveys.csv')
fwrite(plots.dt, 'data/fia/WAORID_fia_plots.csv')
fwrite(cond.dt, 'data/fia/WAORID_fia_cond.csv')
